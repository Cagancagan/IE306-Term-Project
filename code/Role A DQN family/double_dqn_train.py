...
print(f"Double DQN Eğitimi Başlıyor (Seed: {my_seed})...")
for episode in range(config["total_episodes"]):
    ...
    if total_reward > best_reward:
        ...
        torch.save(agent.q_net.state_dict(), f"../../weights/best_double_dqn_seed{my_seed}.pt")

    if episode % 10 == 0: print(f"Bölüm: {episode} | Toplam Ödül: {total_reward:.2f} | Rekor: {best_reward:.2f}")

with open(f"../../logs/double_dqn_seed{my_seed}.csv", "w", newline="") as f:
    writer = csv.writer(f)
